...
if(!isset($_SESSION['email'])){
    if(str_contains($path, 'pages')){
        header("Location:../auth/login.php?err_msg=You must register yourself in system first.");
    }else{
        header("Location:./pages/auth/login.php?err_msg=You must register yourself in system first.");
    }
    exit();
}
?>
