...
if(isset($_POST['editPost'])){
    if (empty(trim($_POST['title']))) {
        $invalidInputTitle = 'Postname is Necessary!';
    }

    if (empty(trim($_POST['author']))) {
        $invalidInputAuthor = 'Author-name is Necessary!';
    }

    if (empty(trim($_POST['body']))) {
        $invalidInputBody = 'Post-description is Necessary!';
    }
    ...
    $categoryId = $_POST['categoryId'];
        ...
        if (!empty(trim($_FILES['image']['name']))){
            ...
            if (move_uploaded_file($tmpName, "../../../uploads/posts/$nameImage")) {
                $postUpdate = $db->prepare("UPDATE posts SET title =:title, author =:author, category_id =:category_id, body =:body, image =:image WHERE id =:id ");
                $postUpdate->execute(['title' => $title, 'author' => $author, 'category_Id' => $categoryId, 'body' => $body, 'image' => $nameImage]);
                
            } else {
                echo "Upload Error";
            }
        } else {
                $postUpdate = $db->prepare("UPDATE posts SET title =:title, author =:author, category_id =:category_id, body =:body WHERE id =:id ");
                $postUpdate->execute(['title' => $title, 'author' => $author, 'category_Id' => $categoryId, 'body' => $body, 'image' => $nameImage]);
                }    
    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 mb-5">
            <div
                ...
                <h1 class="fs-3 fw-bold"> Article editing</h1>
            </div>
            ...
            <div class="mt-4">
                <form method="post" class="row g-4" enctype="multipart/form-data">
                    ...
                    <div class="col-12">
                        <button name="aditPost" type="submit" class="btn btn-dark">
                            Edit
                        </button>
                    </div>
                </form>
